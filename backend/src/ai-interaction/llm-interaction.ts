...
const systemPrompt: PromptPair = [
  "system",
  `You are an RFP proposal creation assistant. Your goal is to extract and build a complete RFP proposal from the user's conversation.
...
export class ChainBuilder {
    ...
    if (conversationHistory && conversationHistory.length > 0) {
      for (const msg of conversationHistory) {
        if (msg.role === "user") {
          ...
          this.prompts.push(["assistant", msg.content]);
        }
      }
    }
  }
  ...
  async interactLLM(
    ...
    const result = await ChatPromptTemplate
